...
type AppFirewallReconciler struct {
	client.Client
	Log       logr.Logger
	ClientSet *xcclientset.ClientSet
}

// +kubebuilder:rbac:groups=xc.f5.com,resources=appfirewalls,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=xc.f5.com,resources=appfirewalls/status,verbs=get;update;patch
// +kubebuilder:rbac:groups=xc.f5.com,resources=appfirewalls/finalizers,verbs=update

func (r *AppFirewallReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("appfirewall", req.NamespacedName)
...
func (r *AppFirewallReconciler) handleCreate(ctx context.Context, log logr.Logger, cr *v1alpha1.AppFirewall, xc xcclient.XCClient, xcNS string) (ctrl.Result, error) {
	create := buildAppFirewallCreate(cr, xcNS)
	result, err := xc.CreateAppFirewall(ctx, xcNS, create)
	if err != nil {
		return r.handleXCError(ctx, log, cr, err, "create")
	}

	log.Info("created XC app firewall", "name", cr.Name, "xcNamespace", xcNS)
	r.setStatus(cr, true, true, v1alpha1.ReasonCreateSucceeded, "XC app firewall created", result)
...
func (r *AppFirewallReconciler) handleUpdate(ctx context.Context, log logr.Logger, cr *v1alpha1.AppFirewall, xc xcclient.XCClient, xcNS string, current *xcclient.AppFirewall) (ctrl.Result, error) {
	desiredJSON, err := buildAppFirewallDesiredSpecJSON(cr, xcNS)
	if err != nil {
	...
	if !needsUpdate {
		r.setStatus(cr, true, true, v1alpha1.ReasonUpToDate, "XC app firewall is up to date", current)
		if err := r.Status().Update(ctx, cr); err != nil {
			return ctrl.Result{}, err
		}
		return ctrl.Result{}, nil
	...
	replace := buildAppFirewallReplace(cr, xcNS, current.Metadata.ResourceVersion)
	result, err := xc.ReplaceAppFirewall(ctx, xcNS, cr.Name, replace)
	if err != nil {
		return r.handleXCError(ctx, log, cr, err, "update")
	}

	log.Info("updated XC app firewall", "name", cr.Name, "xcNamespace", xcNS)
	r.setStatus(cr, true, true, v1alpha1.ReasonUpdateSucceeded, "XC app firewall updated", result)
	if err := r.Status().Update(ctx, cr); err != nil {
		return ctrl.Result{}, err
	}
	return ctrl.Result{}, nil
...
func (r *AppFirewallReconciler) handleDeletion(ctx context.Context, log logr.Logger, cr *v1alpha1.AppFirewall) (ctrl.Result, error) {
	if !controllerutil.ContainsFinalizer(cr, v1alpha1.FinalizerXCCleanup) {
		return ctrl.Result{}, nil
	}
	...
	if policy != v1alpha1.DeletionPolicyOrphan {
		...
		log.Info("deleted XC app firewall", "name", cr.Name, "xcNamespace", xcNS)
	} else {
		log.Info("orphaning XC app firewall", "name", cr.Name)
	}
...
func (r *AppFirewallReconciler) handleXCError(ctx context.Context, log logr.Logger, cr *v1alpha1.AppFirewall, err error, operation string) (ctrl.Result, error) {
	switch {
...
func (r *AppFirewallReconciler) setStatus(cr *v1alpha1.AppFirewall, ready, synced bool, reason, message string, xcObj *xcclient.AppFirewall) {
...
func (r *AppFirewallReconciler) setCondition(cr *v1alpha1.AppFirewall, condType string, status metav1.ConditionStatus, reason, message string) {
